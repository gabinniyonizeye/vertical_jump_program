<template>
  <div>
    ...
    <div v-if="loadingUsers" class="loading-msg">Loading users…</div>
    ...
    <template v-else>
      ...
      <div v-for="u in pending" :key="u.uid" class="user-card card">
        <div class="user-info">
        ...
        <div class="user-actions">
          <button class="confirm-btn" @click="confirm(u.uid)">✅ Confirm</button>
          <button class="reject-btn" @click="reject(u.uid)">❌ Reject</button>
        </div>
      </div>
      ...
      <div v-for="u in confirmed" :key="u.uid" class="user-card card confirmed">
        <div class="user-info">
          <div class="user-name">{{ u.name }}</div>
          <div class="user-email">{{ u.email }}</div>
        ...
        <div class="user-actions">
          <button class="reject-btn" @click="reject(u.uid)">Revoke</button>
          <button class="delete-btn" @click="del(u.uid)">🗑️</button>
        </div>
      </div>

      <div class="section-label">❌ Rejected ({{ rejected.length }})</div>
      <div v-if="!rejected.length" class="empty-msg">No rejected users.</div>
      <div v-for="u in rejected" :key="u.uid" class="user-card card rejected">
        <div class="user-info">
          <div class="user-name">{{ u.name }}</div>
          <div class="user-email">{{ u.email }}</div>
        </div>
        <div class="user-actions">
          <button class="confirm-btn" @click="confirm(u.uid)">Re-approve</button>
          <button class="delete-btn" @click="del(u.uid)">🗑️</button>
        </div>
      </div>
    </template>
...
async function confirm(uid) { await adminConfirm(uid); refresh() }
async function reject(uid)  { await adminReject(uid);  refresh() }
async function del(uid) {
  if (window.confirm('Delete this user permanently?')) { await adminDelete(uid); refresh() }
}
...
<style scoped>
.admin-header { margin-bottom: 20px; border-left: 3px solid var(--accent); }
.admin-title { font-size: 20px; font-weight: 700; color: var(--text-h); margin-bottom: 4px; }
.admin-sub { font-size: 13px; color: var(--text); }
.loading-msg { color: var(--text); padding: 20px 0; font-size: 14px; }
.section-label {
...
.user-card {
  ...
  align-items: center; gap: 16px; margin-bottom: 10px; flex-wrap: wrap;
}
...
.paid-badge { padding: 2px 8px; border-radius: 20px; font-size: 11px; font-weight: 600; }
...
.confirm-btn {
  ...
  padding: 7px 14px; font-size: 13px; font-weight: 600; transition: opacity 0.2s;
}
.confirm-btn:hover { opacity: 0.85; }
.reject-btn {
  background: #f9731622; color: var(--accent); border: 1px solid var(--accent);
